...
- **Runtime/model/egress are already resolved** in `packages/cli/src/profile.ts`
  and sent in each request. The CLI already _knows_ the runtime and model — it
...
- **Greeting detection** (`src/greeting.ts`): normalize `argv.slice(2).join(" ")`
  and match `^(hi|hey|hello|yo)\b[\s,!.]*(jerry)?$/i`. A greeting is _only_ a
...
| `/dryrun <task>`     | Resolve the plan (model + tools that _would_ run) without sending — honors `plan.md` §4's `--dry-run` promise |
...
The user's core requirement — _no assumptions when evidence is missing_ — is an
