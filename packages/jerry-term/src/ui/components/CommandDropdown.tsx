...
export function filterCommands(commands: Command[], filter: string): Command[] {
  ...
  return commands.filter(
    (cmd) =>
      ...
      cmd.aliases?.some((alias) => alias.toLowerCase().startsWith(lowerFilter))
  );
}
...
export function useCommandDropdown(
  ...
  const complete = React.useCallback((): string | null => {
    ...
    const cmd = filteredCommands[clampIndex(selectedIndex, filteredCommands.length)];
  ...
  const onKey = React.useCallback(
    ...
    [isOpen, filteredCommands, complete, setInputValue, selectedIndex, onExecute]
  );
  ...
  if (commands.length === 0) {
    return (
      <box
        ...
        paddingRight={1}
      >
        <text style={{ fg: colors.textMuted }}>No matching commands</text>
      </box>
    );
  }
    ...
    >
      {visibleCommands.map((cmd, index) => {
        ...
        return (
          ...
          >
            <text
              style={{ fg: isSelected ? colors.accent : colors.textPrimary }}
            >
              {paddedName}
            </text>
